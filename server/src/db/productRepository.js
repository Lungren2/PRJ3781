...
export function getAllProductRequests(filters = {}) {
  const clauses = []
  const params = {}
...
export function getProductRequestById(id) {
  const rows = selectRequests('pr.id = @id', { id })
  return hydrate(rows)[0] ?? null
}

export function updateProductRequest(id, updates) {
  const assignments = []
...
export function assignStudent(requestId, userId) {
  getDb()
    .prepare(
